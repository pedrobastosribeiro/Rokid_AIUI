...
export class RemoteModel {
  ...
  // Whether there is a request this client could actually abort right now.
  //
  // Distinct from `isConfigured()`, which only says a key exists. The stop path
  // needs this one: after a remote failure the host fallback runs with no
  // remote task left, and a stop keyed on configuration would report the turn
  // cancelled while the host call keeps going -- invisible, and free to
  // interleave with whatever turn starts next on the same session.
  isPending() {
