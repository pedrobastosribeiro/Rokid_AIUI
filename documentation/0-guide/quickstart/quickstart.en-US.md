...
2.Import an AIUI project from a local folder, a local .aix file, or a GitHub subdirectory. Craft treats everything after `/tree/` as one git ref, so a URL like `/tree/main/samples/pt-br` fails. For the pt-BR glasses agent, paste `https://github.com/OWNER/REPO/tree/cursor/pt-br-craft-e686` instead, replacing `OWNER/REPO` with this GitHub repository.
